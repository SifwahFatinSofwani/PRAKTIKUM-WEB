document.addEventListener('DOMContentLoaded', function() {
    ...
    if (sidebarToggle) {
        ...
        if (mobileOverlay) mobileOverlay.addEventListener('click', handleSidebarToggle);
        if (window.innerWidth <= 992) {
    ...
    if (themeSwitch && body) {
        ...
        themeSwitch.addEventListener('change', () => {
            if (themeSwitch.checked) enableDarkMode();
            else disableDarkMode();
        });
    }

    // Logika untuk lihat/sembunyikan password di halaman login
...
function previewFile(inputId, textId, previewId) {
    ...
    if (input.files && input.files.length > 0) {
        ...
        textElement.textContent = 'Pilih File...';
        previewElement.src = '';
        previewElement.style.display = 'none';
    }
}